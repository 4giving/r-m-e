...
export default class LinkToolbar extends React.Component<Props> {
	...
	handleOnCreateLink = async (title: string) => {
		...
		dispatch(
			view.state.tr.insertText(title, from, to).addMark(from, to + title.length, state.schema.marks.link.create({ href }))
		);
	...
	handleOnSelectLink = ({ href, title }: { href: string; title: string; from: number; to: number }) => {
	...
	render() {
		...
		return (
			<FloatingToolbar ref={this.menuRef} active={isActive(this.props)} {...rest}>
				{isActive(this.props) && (
					<LinkEditor
						from={selection.from}
						to={selection.to}
