...
class BlockMenu extends React.Component<Props, State> {
	...
	calculatePosition(props) {
		...
		if (!props.isActive || !paragraph.node || !paragraph.node.getBoundingClientRect) {
			return {
				left: -1000,
				top: 0,
				bottom: undefined,
	...
	render() {
		...
		return (
			<Portal>
				<Wrapper id="block-menu-container" active={isActive} ref={this.menuRef} {...positioning}>
					{insertItem ? (
						<LinkInputWrapper>
							<LinkInput
								type="text"
...
const LinkInput = styled(Input as any)`
	height: 36px;
	width: 100%;
